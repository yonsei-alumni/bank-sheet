# 통장 거래내역 변환기
신한은행 모임통장에서는 계좌를 개설한 모임장만이 입금내역을 홈페이지에서 엑셀로 다운로드를 받을 수 있슶니다.
이 프로그램은 모임장이 아닌 사람도 입급내역을 단순반복 작업없이 Excel로 변화할 수 있도록 도와줍니다.
...
7. 복사된 내용을 메모장 같은데 붙여넣고 인식이 쉬운 파일명으로, 확장자는 .html으로 저정합니다.
